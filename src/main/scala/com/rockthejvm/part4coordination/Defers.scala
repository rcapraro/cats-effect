...
object Defers extends IOApp.Simple {
  ...
  /** Exercises:
    ...
    *     - on completion (with any status), each IO needs to complete that Deferred (hint: use a finalizer from the Resources lesson) (hint2: use a guarantee
    *       call to make sure the fibers complete the Deferred)
